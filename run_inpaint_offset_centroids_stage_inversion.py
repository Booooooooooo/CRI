...
def project(
    ...
    *,
    ...
    # Load VGG16 feature detector.
    # vgg16_url = 'https://api.ngc.nvidia.com/v2/models/nvidia/research/stylegan3/versions/1/files/metrics/vgg16.pkl'
    vgg16_url = '/youtu_action_data/boooowang/pre_trained_models/vgg16.pkl'
    vgg16 = metric_utils.get_feature_detector(vgg16_url, device=device)

    # Features for target image.
    target_images = target.unsqueeze(0).to(device).to(torch.float32)
    # target_images_ori = target_images.clone()
    if target_images.shape[2] > 256:
        target_images = F.interpolate(target_images, size=(256, 256), mode='area')
    target_features = vgg16(target_images, resize_images=False, return_lpips=True)
    ...
    if centroids_path != "":
        ...
        if center_idx != -1:
            ...
            center_images = pre_process(center_images)
            ...
            # print(center_features.shape, target_features.shape)
    ...
    for step in range(num_steps):
        ...
        synth_features = vgg16(synth_images, resize_images=False, return_lpips=True)
        lpips_loss = (target_features - synth_features).square().sum()
        l2_loss = l2_criterion(synth_images, target_images) * 100
    ...
    # print(gen_im_lr, target_images_ori)
    if ema:
        ema.store()
...
@click.option('--reg-w', help='weight of reg loss', type=float, default=1,)
...
def run_projection(
    ...
    target_tensor = target_tensor.permute(0, 2, 3, 1).clamp(0, 255).to(torch.uint8)[0].cpu().numpy()
    PIL.Image.fromarray(target_tensor, 'RGB').save(f'{outdir}/target.png')
